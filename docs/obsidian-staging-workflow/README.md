...
当前 dry-run 版本不调用微信接口，也不抓取公众号正文。微信资料进入仓库有两种方式：
...
- `scripts/obsidian_capture.py`：命令行入口。
- `shared/obsidian_staging.py`：分类、frontmatter、文件名和 Markdown 生成逻辑。
- `tests/test_obsidian_staging.py`：本地 dry-run 行为测试。
