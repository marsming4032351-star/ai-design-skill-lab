...
这份文档整理当前 Codex 环境中可用的 Skills：什么时候会触发、适合什么任务、使用时要注意什么。你通常不用手动运行 Skill，只要需求命中场景，我会自动读取对应说明并执行。
</callout>

## 使用方式总览

你可以直接描述需求，也可以显式指定 Skill。
...
| 任务类型 | 优先使用 |
|---|---|
| 创意、功能、方案设计前 | `superpowers:brainstorming` |
| 代码、调试、测试、提交前 | `karpathy-guidelines` 与相关 Superpowers |
| 飞书 / Lark 操作 | 对应 `lark-*` skill |
| Obsidian 文件 / vault 操作 | 对应 `obsidian-*` skill |
| PDF、PPT、Word、Excel 等转 Markdown | `markitdown` |
| 网页清洗成 Markdown | `defuddle` |
| 真实浏览器检查网页 / UI | `playwright` |
...
| 你可以这样说 | 会触发 |
|---|---|
| 把这个 PPT 转成 Markdown 并总结重点 | `markitdown` |
| 帮我头脑风暴一个品牌策划案 | `superpowers:brainstorming` |
| 打开这个 HTML 看看效果，并截图验证 | `playwright` |
| 把这份 Markdown 发到飞书文档里 | `lark-doc`，如遇权限问题再用 `lark-shared` |
| 在我的 Obsidian 里查一下聚贤堂相关笔记 | `obsidian-cli`，需要写 Obsidian 格式时再用 `obsidian-markdown` |
| 这个测试失败了，帮我修 | `superpowers:systematic-debugging`、`superpowers:test-driven-development`、`superpowers:verification-before-completion`、`karpathy-guidelines` |
